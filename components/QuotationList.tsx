...
const QuotationList: React.FC<QuotationListProps> = ({ 
  ...
  const handleShareWhatsApp = (qt: Quotation) => {
    const client = getClient(qt.clientId);
    const total = qt.items.reduce((sum, item) => sum + (item.qty * item.rate * (1 + item.taxRate / 100)), 0);
    const text = `Hi ${client?.name || 'Customer'},\n\nPlease find the quotation ${qt.number} for the amount of ${formatCurrency(total)}. \n\nRegards,\nCraft Daddy`;
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex justify-end gap-1">
                      ...
                      <div className="relative group">
                          ...
                          <div className="hidden group-hover:flex absolute right-0 top-full mt-1 bg-white border border-gray-100 shadow-2xl rounded-2xl z-50 py-2 w-56 flex-col text-left overflow-hidden">
                              <button onClick={() => onConvertToInvoice(qt)} className="flex items-center gap-3 px-4 py-3 text-sm text-indigo-700 hover:bg-indigo-50 hover:text-indigo-800 font-bold transition border-b border-gray-50">
                                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" /></svg>
                                  Convert to Invoice
                              </button>
                              <button onClick={() => onDuplicate(qt)} className="flex items-center gap-3 px-4 py-3 text-sm text-gray-700 hover:bg-indigo-50 hover:text-indigo-600 font-bold transition">
                                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7v8a2 2 0 002 2h6M8 7V5a2 2 0 012-2h4.586a1 1 0 01.707.293l4.414 4.414a1 1 0 01.293.707V15a2 2 0 01-2 2h-2M8 7H6a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2v-2" /></svg>
                                  Duplicate
                              </button>
                              <button onClick={() => onEdit(qt)} className="flex items-center gap-3 px-4 py-3 text-sm text-gray-700 hover:bg-indigo-50 hover:text-indigo-600 font-bold transition">
                                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" /></svg>
                                  View / Print
                              </button>
                              <div className="h-px bg-gray-100 my-1 mx-2"></div>
                              <button onClick={() => handleShareWhatsApp(qt)} className="flex items-center gap-3 px-4 py-3 text-sm text-emerald-600 hover:bg-emerald-50 font-bold transition">
                              ...
                              <div className="h-px bg-gray-100 my-1 mx-2"></div>
                              <button onClick={() => onDelete(qt.id)} className="flex items-center gap-3 px-4 py-3 text-sm text-red-500 hover:bg-red-50 font-bold transition">
